  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        ...
        >
          <img src={LOGO_URL} alt="Kammavaari" style={{ height: '32px' }} />
          <p className="font-accent text-[14px] hidden sm:block" style={{ color: 'rgba(201,168,76,0.85)' }}>
        ...
        {/* Tab Switcher */}
        <div className="flex" style={{ padding: '0 clamp(20px,5vw,32px)', borderBottom: '1px solid rgba(201,168,76,0.15)' }}>
          {(["login", "signup"] as const).map((m) => (
            <button
              key={m}
              onClick={() => { setMode(m); setOtpSent(false); }}
              className="flex-1 font-body text-[14px] transition-all"
              style={{
                padding: '14px 0',
                background: 'none',
                border: 'none',
        ...
        {/* Form Body */}
        <div style={{ padding: 'clamp(20px,4vw,32px) clamp(20px,5vw,32px) clamp(16px,3vw,24px)' }}>
          {mode === "login" ? (
            <div className="space-y-4">
              <div>
                <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>Mobile Number / Email</label>
              ...
              {otpSent && (
                <div style={{ animation: 'fadeUp 0.3s var(--ease-luxury)' }}>
                  <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>Enter OTP</label>
              ...
              <button
                className="w-full font-body text-[14px] font-semibold transition-all"
                style={{
                  height: '48px',
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                <span className="font-body text-[12px]" style={{ color: '#C4B49E' }}>OR</span>
                <div className="flex-1 h-[1px]" style={{ background: 'rgba(201,168,76,0.15)' }} />
              </div>

              <button
                className="w-full flex items-center justify-center gap-2.5 font-body text-[14px] transition-all"
                style={{
                  height: '46px',
                  background: 'white',
                ...
                onMouseEnter={e => { e.currentTarget.style.background = '#FBF5EB'; }}
            ...
            <div className="space-y-4">
              <div>
                <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>Profile Created By</label>
              ...
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))' }}>
                <div>
                  <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>First Name</label>
                  <input type="text" placeholder="First name" style={inputStyle} onFocus={focusHandler} onBlur={blurHandler} />
                </div>
                <div>
                  <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>Last Name</label>
                  <input type="text" placeholder="Last name" style={inputStyle} onFocus={focusHandler} onBlur={blurHandler} />
                </div>
              </div>
              <div>
                <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>I am looking for</label>
                <div className="grid grid-cols-2 gap-3">
                  {(["Bride", "Groom"] as const).map((g) => (
                    <button
                      key={g}
                      onClick={() => setGender(g)}
                      className="font-body text-[14px] font-medium transition-all"
                      style={{
              ...
              <div>
                <label className="font-body text-[12px] tracking-[0.08em] uppercase block mb-2" style={{ color: '#7A6550', fontWeight: 400 }}>Mobile Number</label>
